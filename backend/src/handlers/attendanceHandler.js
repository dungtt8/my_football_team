...
const createSession = async (req, res) => {
    try {
        const { session_date, location, session_type, description, check_in_deadline } = req.body;
        ...
        if (check_in_deadline) {
            ...
            if (deadlineDate <= sessionDate) {
                throw new ValidationError('check_in_deadline must be after session_date');
            }
        }
        ...
        logger.info('Creating attendance session', {
            ...
            check_in_deadline: deadlineDate ? deadlineDate.toISOString() : null
        });
...
const memberCheckIn = async (req, res) => {
    try {
        ...
        // Validate check-in deadline
        if (session.check_in_deadline && new Date() > new Date(session.check_in_deadline)) {
            throw new ValidationError('Check-in deadline has passed');
        }
